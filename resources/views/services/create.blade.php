        ...
        <div class="max-w-2xl mx-auto sm:px-2 lg:px-2">
            ...
            <div class="bg-white p-6 rounded-lg">
                ...
                <div class="mb-4 flex justify-between items-center">
                    <h3 class="text-lg font-semibold mb-0 text-[#164272]">New Services</h3>
                ...
                <form action="{{ route('services.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-control w-full mb-4">
                        <label class="label">
                            <span class="label-text text-gray-800 bg-white">Service Image</span>
                        </label>
                        <input type="file" name="service_img" class="w-full text-sm bg-white text-gray-800" accept="image/*">
                        @error('service_img')
                    ...
                    <div class="form-control w-full mb-4">
                        ...
                        <fieldset class="fieldset">
                            ...
                            <input name="service_name" id="service_name" type="text" class="input w-full bg-white input-bordered text-sm text-gray-800" placeholder="Enter Service Name" />
                        </fieldset>
                        @error('service_name')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <fieldset class="fieldset">
                        ...
                        <input name="service_desc" id="service_desc" type="text" class="input w-full bg-white input-bordered text-sm text-gray-800" placeholder="Enter Service Description" />
                    </fieldset>
                    @error('service_desc')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                    ...
                    <div class="form-control w-full mb-4">
                        ...
                        <fieldset class="fieldset">
                            ...
                            <select name="calculate_by" id="calculate_by" class="text-gray-800 select select-bordered w-full text-sm bg-white" required>
                                <option value="piece" selected>Pieces</option>
                    ...
                    <div class="bypieces">
                        ...
                        <div class="form-control w-full mb-4">
                            ...
                            <input type="number" name="pieces_price" id="pieces_price" class="input input-bordered w-full text-gray-800 bg-white dark:bg-white text-sm" step="0.01" placeholder="Enter Price" required>
                        </div>
                        @error('pieces_price')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="byweight" style="display:none;">
                        <label class="label">
                            <span class="label-text text-gray-800">Service Bag</span>
                        </label>
                        <div id="bag-container">
                            <!-- Default Bag Card will be injected here -->
                        </div>
                    ...
                    <div class="form-control mt-6">
                        <button type="submit" class="btn bg-[#ffd65b] border-0 text-[#164272] hover:bg-[#164272] hover:text-white w-full">Create Service</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        $(document).ready(function () {
            ...
            $('#calculate_by').on('change', function () {
                ...
                if (selected === 'piece') {
                    ...
                    $('.bypieces').show();
                    $('.byweight').hide();
                    $('#bag-container').html(''); // Clear weight cards
                } else {
                    $('#pieces_price').removeAttr('required');
                    $('.bypieces').hide();
                    $('.byweight').show();
                    $('#bag-container').html(getBagCard(0)); // Add default
                }
            });
            ...
            // Trigger on load in case value is already 'weight'
            $('#calculate_by').trigger('change');

            $('form').on('submit', function (e) {
                const calculateBy = $('#calculate_by').val();
                if (calculateBy === 'weight') {
                    let allValid = true;
                    let priceFields = $('input[name="price[]"]');
                    ...
                    if (!allValid) {
                        e.preventDefault();
                        alert('Please fill in the price for all bag entries.');
                    }
                }
            });
