    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-lg">
                ...
                <form method="GET" action="{{ route('deliver.order.index') }}" class="flex flex-wrap gap-4 px-6 pb-4">
                    ...
                    <a href="{{ route('deliver.order.index') }}" class="text-sm underline mt-1 text-[#164272]">Reset</a>
                </form>
                                    ...
                                    <tr class="text-gray-800">
                                        ...
                                        <td class="border border-gray-300 px-4 py-2 capitalize">
                                            @if ($order->order_status !== 'completed')
                                                <form action="{{ route('deliver.order.updateStatus', $order->id) }}" method="POST" class="inline update-status-form">
                                                    @csrf
                                                    @method('PATCH')
                                                    <select name="status" class="mt-1 text-sm rounded border-gray-300 text-gray-700 status-select">
                                                        <option value="pick up" {{ $order->order_status === 'pick up' ? 'selected' : '' }}>Pick Up</option>
                                        ...
                                        <td class="border border-gray-300 px-4 py-2">
                                            <a href="{{ route('deliver.order.show', $order->id) }}"
